...
$coursesResult = $con->query("SELECT DISTINCT course_name FROM courses WHERE user_id = $user_id");
if (!$coursesResult) {
    die("Error retrieving courses: " . $con->error);
}
...
$sectionsResult = $con->query("SELECT DISTINCT section_name FROM sections WHERE user_id = $user_id");
if (!$sectionsResult) {
    die("Error retrieving sections: " . $con->error);
}
...
$subjectsResult = $con->query("SELECT DISTINCT subject_name FROM subjects WHERE user_id = $user_id");
if (!$subjectsResult) {
    die("Error retrieving subjects: " . $con->error);
}

// Fetch subject names into an array
$subjects = [];
...
$query = "
    SELECT students.*, courses.course_name, sections.section_name, subjects.subject_name 
    FROM students 
...
if (!empty($selectedCourse)) {
    $query .= " AND courses.course_name = '$selectedCourse'";
}
...
if (!empty($selectedSubject)) {
    $query .= " AND subjects.subject_name = '$selectedSubject'";
}
...
<div class="container">
    <h2>STUDENTS LIST</h2>
    <a href="add_student.php" class="add-student-form">Add New Student</a>
    ...
    <form method="GET" action="students.php">
        ...
        <select name="subject" id="subject" onchange="this.form.submit()">
            ...
            <?php foreach ($subjects as $subject): ?>
                <option value="<?php echo urlencode($subject); ?>" <?php if (isset($_GET['subject']) && $_GET['subject'] == $subject) echo 'selected'; ?>><?php echo $subject; ?></option>
            <?php endforeach; ?>
        </select>
    ...
    <table>
        ...
        if ($result->num_rows === 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<a href='delete_student.php?id={$row['id']}' class='delete' onclick='return confirm(\"Are you sure?\")'>Delete</a>";
                echo "<a href='student_activities.php?student_id={$row['id']}'>Records</a>";
                echo "</td>";
                echo "</tr>";
            }
        }
